...
class _BuilderSlot:
    ...
    def __init__(self, builder_class: type, root_tag: str,
                 compiler_module: Any, output: Path | None) -> None:
        self.store = Bag(builder=builder_class)
    ...
    def compile_to_dict(self) -> dict[str, Any]:
        """Compile and track resolved pointer paths."""
        self.resolved_paths.clear()
        result = self.compiler.compile_to_dict(self.root, self.store.builder)
        # Collect paths resolved during this compilation
        self.resolved_paths = _collect_resolved_paths(self.root, self.store.builder)
        return result
    ...
    def depends_on(self, changed_path: str) -> bool:
        ...
        if not self.resolved_paths:
            return True  # never compiled yet → must compile
...
def _collect_resolved_paths(root_node: Any, builder: Any) -> set[str]:
...
class ScribaApp:
    ...
    def __init__(self,
        ...
        # Initial compile to populate dependency tracking
        for slot in self._slots.values():
    ...
    def _init_traefik(self, output: str | Path | None) -> None:
        ...
        try:
            ...
            self._slots["traefik"] = _BuilderSlot(
                TraefikBuilder, "traefik", traefik_compiler,
                Path(output) if output else None,
            )
        except ImportError:
            pass  # genro-traefik not installed
    ...
    def _init_compose(self, output: str | Path | None) -> None:
        ...
        try:
            ...
            self._slots["compose"] = _BuilderSlot(
                ComposeBuilder, "compose", compose_compiler,
                Path(output) if output else None,
            )
        except ImportError:
            pass  # genro-compose not installed
    ...
    def to_yaml(self, builder_name: str,
        ...
        Args:
            builder_name: "traefik" or "compose".
    ...
    def to_yaml_all(self) -> dict[str, str]:
        ...
        Returns:
            Dict of {builder_name: yaml_string}.
        """
        result = {}
    ...
    def _on_data_changed(self, node: Any = None, pathlist: list | None = None,
        ...
        # Reconstruct the changed path from pathlist
